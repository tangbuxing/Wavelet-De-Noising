# -*- coding: utf-8 -*-
"""
Created on Sat Mar 20 16:00:48 2021
...
def mad(x):
    res_mad = 1.4826*np.median(np.abs(x - np.median(x)))
    return res_mad
...
def grid_extend(grid):
    ...
    if len_lat.is_integer() and len_lon.is_integer():
        print("数据大小符合计算要求，调用denoise_dwt_2d")
    else:
        #如果纬度方向不满足
...
if __name__ == "__main__":
    ...
    X,Y = np.meshgrid(x, y)
    X_Y = grid_extend(grid = X)
    #plt.imshow(X_Y)

    aa_mad1 = mad(x = np.arange(1, 10))
